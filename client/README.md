...
| 好友 | 分页好友列表、搜索用户名/昵称后点击添加好友、收到申请列表、点击同意/拒绝、二次确认删除 | 搜索排除自己，显示已是好友/已申请状态；打开好友页面自动加载待处理申请，每页 10 条，可滚动及翻页；不展示或手工填写申请 ID |
| 消息 | 点击好友进入聊天、最近会话、消息气泡、发送文本、手动刷新 | 每次获取完整历史，无实时推送；已记录联系人删除后仍可从最近会话读历史，但不能发送 |
| 动态 | 分页广场、发布、评论提交与展开查看、点赞/取消点赞、删除自己的动态 | 所有登录用户可见；历史评论通过 JWT API 分页读取，每页 10 条，评论区域独立滚动，可展开/收起；提交后自动刷新 |
...
- `Scripts/Posts`：PostApi、DTO、PostsView、PostCommentsScrollRect（评论滚动边界交接）。
...
第九阶段第二轮已优化好友条目及消息气泡，点击好友昵称或“聊天”自动选择对象；聊天页不再输入用户 ID。发送申请从好友页“添加好友”进入搜索；收到的申请在好友页自动显示，也可在申请页处理，同意和拒绝均自动刷新列表，删除好友仍需确认。最近联系人通过 PlayerPrefs 按账号和服务器保存基本信息，不保存消息、密码或 JWT；它不等于服务端完整会话列表，无法自动发现本机未记录且已删除的联系人。文件、备份和测试结果见 [好友聊天验证](../docs/unity-chat-validation.md)。

动态评论查看与紧凑布局的验证记录见 [动态体验验证](../docs/post-comments-validation.md)。

用户搜索添加好友的文件清单与验收步骤见 [搜索添加好友验证](../docs/user-search-validation.md)。
